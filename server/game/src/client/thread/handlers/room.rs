...
impl ClientThread {
    ...
    gs_handler!(self, handle_ban_room_player, BanRoomPlayerPacket, packet, {
...

        {
            ...
            let owner_id = room.owner.load(Ordering::Relaxed);
            let is_private_room = room.id != self.game_server.state.room_manager.get_global().id;
            let is_owner = account_id == owner_id;
